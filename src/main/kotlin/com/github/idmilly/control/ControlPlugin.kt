...
class ControlPlugin : JavaPlugin(), Listener {
    ...
    private fun blockPlace(event: BlockPlaceEvent) {
        if (config.getBoolean("event.blockPlace")) { event.isCancelled = true }
    }

    @EventHandler
    private fun blockBreak(event: BlockBreakEvent) {
        if (config.getBoolean("event.blockBreak")) { event.isCancelled = true }
    }

    @EventHandler
    private fun playerInteract(event: PlayerInteractEvent) {
        if (config.getBoolean("event.playerInteract")) { event.isCancelled = true }
    }

    @EventHandler
    private fun playerMove(event: PlayerMoveEvent) {
        if (config.getBoolean("event.playerMove")) { event.isCancelled = true }
    }

    @EventHandler
    private fun playerPortal(event: PlayerPortalEvent) {
        if (config.getBoolean("event.playerPortal")) { event.isCancelled = true }
    }

    @EventHandler
    private fun asyncChat(event: AsyncChatEvent) {
        if (config.getBoolean("event.asyncChat")) { event.isCancelled = true }
    }

    @EventHandler
    private fun inventoryOpen(event: InventoryOpenEvent) {
        if (config.getBoolean("event.inventoryOpen")) { event.isCancelled = true }
    }

    @EventHandler
    private fun playerJoinMessage(event: PlayerJoinEvent) {
        if (config.getBoolean("event.playerJoinMessage")) { event.joinMessage(null) }
    }

    @EventHandler
    private fun playerQuitMessage(event: PlayerQuitEvent) {
        if (config.getBoolean("event.playerQuitMessage")) { event.quitMessage(null) }
    }
    ...
    private fun playerCommandPreprocess(event: PlayerCommandPreprocessEvent) {
        if (config.getBoolean("event.playerCommandPreprocess")) { event.isCancelled = true }
    }
}